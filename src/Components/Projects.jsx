
import { Box, Text, Heading, Flex, SimpleGrid, Link } from "@chakra-ui/react";
import React from 'react'
import style from "./Projects.module.css"

const DeployedLink = {
  maxfashion_D_Link: "https://adorable-kitten-07d0ff.netlify.app/",
  kfc_D_Link: "https://transcendent-stardust-f3081b.netlify.app/",
  nykaa_D_Link: "https://scintillating-meringue-fdf589.netlify.app/",
  healthkart_D_Link: "https://nutribest-mocha.vercel.app/",
  advanceAutoparts_D_Link: "https://advance-auto-parts.vercel.app/",
  chandaKaudi_D_Link: "https://chanda-kaudi-game.vercel.app/",
};

const GithubLink = {
  maxfashionGithub: "https://github.com/Rishav048/phobic-lumber-7659",
  kfcGithub: "https://github.com/Rishav048/-unique-stretch-3372",
  nykaaGithub: "https://github.com/Rishav048/rustic-bone-9818",
  healthkartGithub: "https://github.com/akaushik509/internal-appliance-3345",
  advanceAutopartsGithub: "https://github.com/SORDE12/Advance-Spare-Care",
  chandaKaudiGithub: "https://github.com/abhayfaldu/Chanda_kaudi_game",
};

const {
  maxfashion_D_Link,
  kfc_D_Link,
  nykaa_D_Link,
  healthkart_D_Link,
  advanceAutoparts_D_Link,
  chandaKaudi_D_Link
} = DeployedLink;
const {
  maxfashionGithub,
  kfcGithub,
  nykaaGithub,
  healthkartGithub,
  advanceAutopartsGithub,
  chandaKaudiGithub
} = GithubLink;


const Projects = () => {


      return (
        <>
          <div id="projects">
            <Heading textAlign={"center"} color={"blue.600"}>
              Projects
            </Heading>
            <SimpleGrid
              columns={{ base: 1, md: 2, lg: 2 }}
              gap={"40px"}
              className={style.portfolio}
            >
              {/* ................ 1st Project(Chanda Kaudi Game) .................. */}
              <div className={style.portfoliocontent}>
                <div className={style.portfolioimg}>
                  <Box className={style.upperBox}>
                    <Box className={style.projectName}>
                      <Text fontSize={{ base: "20px", md: "19px", lg: "24px" }}>
                        <img
                          className={style.logoImg}
                          src="https://user-images.githubusercontent.com/107506646/222924374-0631c640-ee40-444c-9e3e-f87dc8913e67.png"
                          alt="Logo"
                        />{" "}
                        Chanda Kaudi - Game
                      </Text>
                    </Box>
                    <div className={style.aboutProject}>
                      <p style={{ textAlign: "left", color: "blue" }}>
                        About Project :
                      </p>
                      <p style={{ fontSize: "14px", textAlign: "left" }}>
                        CHANDA KAUDI, also known as Chaupar or Chaubara, is a
                        traditional Indian board game that has been enjoyed
                        enjoyed in India for centuries, and it is still played
                        by many people today. players can connect with each
                        other by real time chats while playing the game
                        together. This was a collaborative Project which is made
                        by 4 team members within 2 Days (48 hrs).
                      </p>
                    </div>
                    <br></br>
                    <img
                      className={style.projectimg}
                      src="https://masai-course.s3.ap-south-1.amazonaws.com/editor/uploads/2023-03-05/Screenshot%20%28148%29_188726.png"
                      alt="Chanda Kaudi SS"
                    />
                  </Box>
                  <br></br>
                  <Box className={style.Techstacks}>
                    <p style={{ textAlign: "left", color: "blue" }}>
                      Tech Stacks :
                    </p>
                    <Text fontSize={{ base: "13px", md: '"15px"' }}>
                      React | Redux | Typescript | Chakra UI | Express | NodeJS
                    </Text>
                  </Box>
                  <Flex mt={"30px"} pt={"20px"} justifyContent={"space-evenly"}>
                    <button className={style.btn}>
                      <Link href={chandaKaudiGithub} isExternal>
                        <img
                          style={{
                            borderRadius: "50%",
                            background: "white",
                            marginRight: "5px",
                            display: "inline",
                            width: "20px",
                            height: "20px",
                          }}
                          src="https://cdn-icons-png.flaticon.com/512/25/25231.png"
                          alt="GitIcon"
                        />{" "}
                        Github
                      </Link>
                    </button>

                    <button className={style.btn}>
                      <Link href={chandaKaudi_D_Link} isExternal>
                        <img
                          style={{
                            borderRadius: "50%",
                            background: "white",
                            marginRight: "5px",
                            display: "inline",
                            width: "20px",
                            height: "20px",
                          }}
                          src="https://cdn-icons-png.flaticon.com/512/0/375.png"
                          alt="PlayIcon"
                        />{" "}
                        Live
                      </Link>
                    </button>
                  </Flex>
                </div>
              </div>
              {/* ................ 2nd Project(Advance Autoparts) .................. */}

              <div className={style.portfoliocontent}>
                <div className={style.portfolioimg}>
                  <Box className={style.upperBox}>
                    <Box className={style.projectName}>
                      <Text fontSize={{ base: "20px", md: "19px", lg: "24px" }}>
                        <img
                          className={style.logoImg}
                          src="https://raw.githubusercontent.com/SORDE12/Advance-Spare-Care/main/frontend/src/Assets/logo.jpeg"
                          alt="Logo"
                        />{" "}
                        Advance Autoparts - Clone
                      </Text>
                    </Box>
                    <div className={style.aboutProject}>
                      <p style={{ textAlign: "left", color: "blue" }}>
                        About Project :
                      </p>
                      <p style={{ fontSize: "14px", textAlign: "left" }}>
                        Advance Auto Parts, Inc. is a leading automotive
                        aftermarket parts provider that serves both professional
                        installer and do-it-yourself customers. Here I
                        Implemented the Admin Side of the website, added
                        functionalities like searching , filtering , and CRUD
                        (Create,Read,Update,Delete) operation on database.
                      </p>
                    </div>
                    <br></br>
                    <img
                      className={style.projectimg}
                      src="https://user-images.githubusercontent.com/108116297/221490829-713675c9-ce32-428b-b11d-2cbf7b95a8af.png"
                      alt="Advance autoparts SS"
                    />
                  </Box>
                  <br></br>
                  <Box className={style.Techstacks}>
                    <p style={{ textAlign: "left", color: "blue" }}>
                      Tech Stacks :
                    </p>
                    <Text fontSize={{ base: "13px", md: '"15px"' }}>
                      React | Redux | Chakra UI | MongoDB | Express | NodeJS
                    </Text>
                  </Box>
                  <Flex mt={"30px"} pt={"20px"} justifyContent={"space-evenly"}>
                    <button className={style.btn}>
                      <Link href={advanceAutopartsGithub} isExternal>
                        <img
                          style={{
                            borderRadius: "50%",
                            background: "white",
                            marginRight: "5px",
                            display: "inline",
                            width: "20px",
                            height: "20px",
                          }}
                          src="https://cdn-icons-png.flaticon.com/512/25/25231.png"
                          alt="GitIcon"
                        />{" "}
                        Github
                      </Link>
                    </button>

                    <button className={style.btn}>
                      <Link href={advanceAutoparts_D_Link} isExternal>
                        <img
                          style={{
                            borderRadius: "50%",
                            background: "white",
                            marginRight: "5px",
                            display: "inline",
                            width: "20px",
                            height: "20px",
                          }}
                          src="https://cdn-icons-png.flaticon.com/512/0/375.png"
                          alt="PlayIcon"
                        />{" "}
                        Live
                      </Link>
                    </button>
                  </Flex>
                </div>
              </div>
              {/* ................ 3rd Project(Health Kart) .................. */}

              <div className={style.portfoliocontent}>
                <div className={style.portfolioimg}>
                  <Box className={style.upperBox}>
                    <div className={style.projectName}>
                      <Text
                        className={style.nameP}
                        fontSize={{ base: "20px", md: "19px", lg: "24px" }}
                      >
                        <img
                          className={style.logoImg}
                          src="https://i.ibb.co/KNkk7GQ/logo-withname.png"
                          alt="Logo"
                        />{" "}
                        HealthKart.com - Clone
                      </Text>
                    </div>
                    <div className={style.aboutProject}>
                      <p style={{ textAlign: "left", color: "blue" }}>
                        About Project :
                      </p>
                      <p style={{ fontSize: "14px", textAlign: "left" }}>
                        HealthKart is an online health and fitness store which
                        sells authentic health supplements, fitness equipment,
                        protein supplements, sports gear and wellness products.
                        Here I Implemented functionalities like Carousel,
                        filter, sorting, pagination, dynamic products per page,
                        adding products.
                      </p>
                    </div>
                    <br></br>
                    <img
                      className={style.projectimg}
                      src="https://user-images.githubusercontent.com/26029251/215933811-50a82010-8114-476c-9627-e2484710c93a.png"
                      alt="Health Kart SS"
                    />
                  </Box>
                  <br></br>
                  <div className={style.Techstacks}>
                    <p style={{ textAlign: "left", color: "blue" }}>
                      Tech Stacks :
                    </p>
                    <p style={{ fontSize: "13px" }}>
                      Next.js | Redux | CSS | Chakra UI
                    </p>
                  </div>
                  <Flex mt={"30px"} pt={"20px"} justifyContent={"space-evenly"}>
                    <button className={style.btn}>
                      <Link href={healthkartGithub} isExternal>
                        <img
                          style={{
                            borderRadius: "50%",
                            background: "white",
                            marginRight: "5px",
                            display: "inline",
                            width: "20px",
                            height: "20px",
                          }}
                          src="https://cdn-icons-png.flaticon.com/512/25/25231.png"
                          alt="GitIcon"
                        />{" "}
                        Github
                      </Link>
                    </button>

                    <button className={style.btn}>
                      <Link href={healthkart_D_Link} isExternal>
                        <img
                          style={{
                            borderRadius: "50%",
                            background: "white",
                            marginRight: "5px",
                            display: "inline",
                            width: "20px",
                            height: "20px",
                          }}
                          src="https://cdn-icons-png.flaticon.com/512/0/375.png"
                          alt="PlayIcon"
                        />{" "}
                        Live
                      </Link>
                    </button>
                  </Flex>
                </div>
              </div>
              {/* ******************4rth (Nykaa)  ********************* */}
              <div className={style.portfoliocontent}>
                <div className={style.portfolioimg}>
                  <Box className={style.upperBox}>
                    <div className={style.projectName}>
                      <img
                        className={style.logoImg}
                        src="https://cdn.iconscout.com/icon/free/png-256/nykaa-3384872-2822953.png"
                        alt="Logo"
                      ></img>
                      <Text
                        fontSize={{ base: "20px", md: "19px", lg: "24px" }}
                        className={style.nameP}
                      >
                        Nykaa - Clone
                      </Text>
                    </div>
                    <div className={style.aboutProject}>
                      <p style={{ textAlign: "left", color: "blue" }}>
                        About Project :
                      </p>
                      <p style={{ fontSize: "14px", textAlign: "left" }}>
                        Nykaa is an Indian e-commerce company, It sells beauty,
                        wellness and fashion products. Here I built the
                        website's Home, Product's, Cart page. Implemented
                        functionalities like Carousel, filter, pagination,
                        dynamic products per page, adding & removing products.
                      </p>
                    </div>
                    <br></br>
                    <img
                      className={style.projectimg}
                      src="https://masai-course.s3.ap-south-1.amazonaws.com/editor/uploads/2023-01-12/Screenshot%20%2845%29_544913.png"
                      alt="NYKAA SS"
                    />
                  </Box>
                  <br></br>
                  <div className={style.Techstacks}>
                    <p style={{ textAlign: "left", color: "blue" }}>
                      Tech Stacks :
                    </p>
                    <p style={{ fontSize: "13px" }}>
                      React | JavaScript | HTML | CSS | Chakra UI{" "}
                    </p>
                  </div>
                  <Flex
                    border={"0px solid red"}
                    mt={"30px"}
                    pt={"20px"}
                    justifyContent={"space-evenly"}
                  >
                    <button className={style.btn}>
                      <Link href={nykaaGithub} isExternal>
                        <img
                          style={{
                            borderRadius: "50%",
                            background: "white",
                            marginRight: "5px",
                            display: "inline",
                            width: "20px",
                            height: "20px",
                          }}
                          src="https://cdn-icons-png.flaticon.com/512/25/25231.png"
                          alt="GitIcon"
                        />{" "}
                        Github
                      </Link>
                    </button>

                    <button className={style.btn}>
                      <Link href={nykaa_D_Link} isExternal>
                        <img
                          style={{
                            borderRadius: "50%",
                            background: "white",
                            marginRight: "5px",
                            display: "inline",
                            width: "20px",
                            height: "20px",
                          }}
                          src="https://cdn-icons-png.flaticon.com/512/0/375.png"
                          alt="PlayIcon"
                        />{" "}
                        Live
                      </Link>
                    </button>
                  </Flex>
                </div>
              </div>

              {/* ................ 5th Project - (KFC) .................. */}
              <div className={style.portfoliocontent}>
                <div className={style.portfolioimg}>
                  <Box className={style.upperBox}>
                    <div className={style.projectName}>
                      <img
                        className={style.logoImg}
                        src="data:image/png;base64,"
                        alt="Logo"
                      />
                      <Text fontSize={{ base: "20px", md: "19px", lg: "24px" }}>
                        KFC - Clone
                      </Text>
                    </div>
                    <Box className={style.aboutProject}>
                      <p style={{ textAlign: "left", color: "blue" }}>
                        About Project :
                      </p>
                      <p style={{ fontSize: "14px", textAlign: "left" }}>
                        An American online food ordering and delivery
                        platform,that specializes in fried chicken. Here I Built
                        the website's Product's page, added search and filter
                        functionality, Adding & Removing products from cart,
                        Helped in making dynamic admin dashboard.
                      </p>
                    </Box>
                    <br></br>
                    <img
                      className={style.projectimg}
                      src="https://masai-course.s3.ap-south-1.amazonaws.com/editor/uploads/2023-01-12/Screenshot%20%2843%29_774980.png"
                      alt="KFC SS"
                    />
                  </Box>
                  <br></br>
                  <Box className={style.Techstacks}>
                    <p style={{ textAlign: "left", color: "blue" }}>
                      Tech Stacks :
                    </p>
                    <p style={{ fontSize: "13px" }}>HTML | CSS | JavaScript</p>
                  </Box>
                  <Flex
                    border={"0px solid red"}
                    mt={"30px"}
                    pt={"20px"}
                    justifyContent={"space-evenly"}
                  >
                    <button className={style.btn}>
                      <Link href={kfcGithub} isExternal>
                        <img
                          style={{
                            borderRadius: "50%",
                            background: "white",
                            marginRight: "5px",
                            display: "inline",
                            width: "20px",
                            height: "20px",
                          }}
                          src="https://cdn-icons-png.flaticon.com/512/25/25231.png"
                          alt="GitIcon"
                        />{" "}
                        Github
                      </Link>
                    </button>

                    <button className={style.btn}>
                      <Link href={kfc_D_Link} isExternal>
                        <img
                          style={{
                            borderRadius: "50%",
                            background: "white",
                            marginRight: "5px",
                            display: "inline",
                            width: "20px",
                            height: "20px",
                          }}
                          src="https://cdn-icons-png.flaticon.com/512/0/375.png"
                          alt="PlayIcon"
                        />{" "}
                        Live
                      </Link>
                    </button>
                  </Flex>
                </div>
              </div>

              {/* ................ 6th Project - (MAX FASHION) .................. */}

              <div className={style.portfoliocontent}>
                <div className={style.portfolioimg}>
                  <Box className={style.upperBox}>
                    <div className={style.projectName}>
                      <Text fontSize={{ base: "20px", md: "19px", lg: "24px" }}>
                        <img
                          className={style.logoImg}
                          src="https://www.landmarkgroup.com/int/sites/default/files/Brand-logo/New%20Max%20Logo-%20Eng%20With%20Outline_1.png"
                          alt="Logo"
                        />{" "}
                        Max Fashion - Clone
                      </Text>
                    </div>
                    <div className={style.aboutProject}>
                      <p style={{ textAlign: "left", color: "blue" }}>
                        About Project :
                      </p>
                      <p style={{ fontSize: "14px", textAlign: "left" }}>
                        Max Fashion is an e-commerce website, offers apparel,
                        footwear and accessories for the entire family - women,
                        men and children. Here I implemented some
                        functionalities like Login, Signup, Product's Page,
                        Filtering, Adding & Removing products from cart, etc.
                      </p>
                    </div>
                    <br></br>
                    <img
                      className={style.projectimg}
                      src="https://masai-course.s3.ap-south-1.amazonaws.com/editor/uploads/2023-01-11/Screenshot%20%2838%29_662577.png"
                      alt="Max Fashion SS"
                    />
                  </Box>
                  <br></br>
                  <div className={style.Techstacks}>
                    <p style={{ textAlign: "left", color: "blue" }}>
                      Tech Stacks :
                    </p>
                    <Text fontSize={{ base: "13px", md: '"15px"' }}>
                      HTML | CSS | JavaScript
                    </Text>
                  </div>
                  <Flex mt={"30px"} pt={"20px"} justifyContent={"space-evenly"}>
                    <button className={style.btn}>
                      <Link href={maxfashionGithub} isExternal>
                        <img
                          style={{
                            borderRadius: "50%",
                            background: "white",
                            marginRight: "5px",
                            display: "inline",
                            width: "20px",
                            height: "20px",
                          }}
                          src="https://cdn-icons-png.flaticon.com/512/25/25231.png"
                          alt="GitIcon"
                        />{" "}
                        Github
                      </Link>
                    </button>

                    <button className={style.btn}>
                      <Link href={maxfashion_D_Link} isExternal>
                        <img
                          style={{
                            borderRadius: "50%",
                            background: "white",
                            marginRight: "5px",
                            display: "inline",
                            width: "20px",
                            height: "20px",
                          }}
                          src="https://cdn-icons-png.flaticon.com/512/0/375.png"
                          alt="PlayIcon"
                        />{" "}
                        Live
                      </Link>
                    </button>
                  </Flex>
                </div>
              </div>
            </SimpleGrid>
          </div>
        </>
      );
}

export default Projects